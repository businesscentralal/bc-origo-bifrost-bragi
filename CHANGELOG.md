...
- Help and documentation moved to <https://bifrost.origo.is> (source repository `businesscentralal/bifrost`), where the English and Icelandic content for every Bifröst app is published together. The `app/docs` and `app/Help` folders were removed from this repository; `help` in `app.json` now points at `https://bifrost.origo.is/en-us/bragi/` and `contextSensitiveHelpUrl` at `https://bifrost.origo.is/{0}/help/bragi/`.
